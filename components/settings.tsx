    ...
    if (saved) {
      try {
        ...
        if (parsed.logos && parsed.logos.length > 0 && typeof parsed.logos[0] === 'object' && parsed.logos[0] && !('opacity' in parsed.logos[0])) {
          parsed.logos = parsed.logos.map((logo: { url: string; size: number; opacity?: number }) => ({ ...logo, opacity: logo.opacity || 100 }))
        }
    ...
    <div className="space-y-6">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const currentLogo = typeof newLogos[index] === 'string' 
                          ...
                          : { ...(newLogos[index] as { url: string; size: number; opacity?: number }), opacity: (newLogos[index] as { url: string; size: number; opacity?: number })?.opacity || 100 }
      ...
      <div className="border-t pt-6 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            {settings.drawSound && settings.drawSound.trim() 
              ? "Custom draw sound is active. Click 'Use Default Ticking Sound' to restore the default."
              : "Upload a custom sound for the roulette ticking effect (max 10MB). Files are saved to the public folder. Default ticking sound will be used if no custom sound is set."}
          </p>
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              <Input
                ...
                placeholder={settings.drawSound && settings.drawSound.trim() 
                  ? "Custom sound is set" 
                  : "Enter sound URL or upload file (default: ticking sound)"}
                className="flex-1"
                disabled={!!(settings.drawSound && settings.drawSound.trim())}
              />
              {settings.drawSound && settings.drawSound.trim() && (
                <Button
                  type="button"
                  ...
                  onClick={() => {
                    ...
                    localStorage.setItem("drawSettings", JSON.stringify(newSettings))
                    onSettingsChange(newSettings)
                    toast.success("Restored to default ticking sound")
                  }}
                  className="whitespace-nowrap"
                >
                  Use Default Tick Sound
                </Button>
              )}
            </div>
            <Input
              ...
              onChange={async (e) => {
                ...
                if (file) {
                  ...
                  try {
                    ...
                    localStorage.setItem("drawSettings", JSON.stringify(newSettings))
                    onSettingsChange(newSettings)
                    toast.success("Custom draw sound uploaded! Default tick sound is now disabled.")
