...
export function SiteHeader({ currentView, onViewChange, entriesCount, prizesCount = 0 }: SiteHeaderProps) {
  ...
  useEffect(() => {
    ...
    if (savedTheme) {
      ...
      // Only set theme if it's different from current - use setTimeout to avoid setState-in-effect warning
      setTimeout(() => {
      ...
      setTimeout(() => {
        setTheme((prev) => prev !== initialTheme ? initialTheme : prev)
      }, 0)
    }
  }, [])
  ...
  const toggleFullscreen = async () => {
    try {
      if (!document.fullscreenElement) {
        ...
        } else if ((document as typeof document & { webkitExitFullscreen?: () => Promise<void> }).webkitExitFullscreen) {
